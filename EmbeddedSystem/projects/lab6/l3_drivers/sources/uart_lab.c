...
void uart_lab__init(uart_number_e uart, uint32_t peripheral_clock_mhz, uint32_t baud_rate) {
  ...
  if (uart == UART_2) {
    /* Config UART2_PIN + Power ON  */
    LPC_SC->PCONP |= (1 << 24);
    gpio__construct_with_function(2, 8, GPIO__FUNCTION_2);
    gpio__construct_with_function(2, 9, GPIO__FUNCTION_2);

    /* Line Control Register- WLS[1:0] + DLAB[7]  */
    LPC_UART2->LCR |= (3 << 0) | (1 << 7);

    /* Enable FIFO  Control Register FIFOEN[0] */
    LPC_UART2->FCR |= (1 << 0);

    /* Config Baud Rate (DLM +DLL) */
    const float errors_fraction = 0.5;
    uint16_t divider = (peripheral_clock_mhz / (16 * baud_rate) + errors_fraction);
    LPC_UART2->DLM = (divider >> 8) & 0xFF;
    LPC_UART2->DLL = (divider >> 0) & 0xFF;
    fprintf(stderr, "UART2 Initialization DONE \n");

  } else if (uart == UART_3) {
    /* Config UART3_PIN + Power ON  */
    LPC_SC->PCONP |= (1 << 25);
    ...
    fprintf(stderr, "UART3 Initialization DONE \n");
  }
}
